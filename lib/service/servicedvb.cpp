...
RESULT eDVBServicePlay::start()
{
	...
	{
			eDebug("[servicedvb:eDVBServicePlay]start m_is_pvr %d", m_is_pvr);
...
{
	ptr = nullptr;
	eDebug("[servicedvb:eDVBServicePlay] timeshift twol");
	if (m_timeshift_enabled || !m_is_pvr)
	{
		if (!m_timeshift_enabled)
		{
			...
			{
				eDebug("[servicedvb:eDVBServicePlay] not enough diskspace for timeshift! (less than 1GB)");
				return -3;
			}
		}
		ptr = this;
...
//		eDebug("[servicedvb:eDVBServicePlay] Subtitle show %d page.pts=%lld pts=%lld diff=%d", type, show_time, pos, diff);
...
void eDVBServicePlay::newDVBSubtitlePage(const eDVBSubtitlePage &p)
{
	if (m_subtitle_widget)
	{
		...
		{
			eDebug("[servicedvb:eDVBServicePlay] Subtitle without PTS and recording");
...
void eDVBServicePlay::setAC3Delay(int delay)
{
	...
	if (m_decoder)
	{
		...
		eDebug("[servicedvb:eDVBServicePlay] Setting audio delay: setAC3Delay, %d + %d", delay,eConfigManager::getConfigIntValue("config.av.generalAC3delay"));
	}
}
...
void eDVBServicePlay::setPCMDelay(int delay)
{
	...
	if (m_decoder)
	{
		...
		eDebug("[servicedvb:eDVBServicePlay] Setting audio delay: setPCMDelay, %d + %d", delay,eConfigManager::getConfigIntValue("config.av.generalPCMdelay"));
	}
}
